...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    ...
    ' ==============================================================
        ' The purpose of this macro is to:
            ' Transform the Arizona Giving (AZ Gives) report into a Salesforce-ready import file.
            ' Standardize and enhance the original AZ Gives report by removing non-essential columns and adding required fields pulled from the AZ Gives website.
            ' Provide the AR Team with all required data to create the Sage Intacct Accounting System import file.
    
    ' ==============================================================
    ' REQUIREMENTS
    ' ==============================================================
        ' AZ Gives disbursement report exported from: https://www.azgives.org/organization/basisschools/disbursements
        ' The Disbursement ID displayed on each AZ Gives disbursement details page.
        ' Any adjustments (prizes) listed on the AZ Gives details pages.
    
    ' ==============================================================
    ' UPDATE LOG (LAST UPDATED: 2026.01.29)
    ' ==============================================================
        ' Original Production Rollout Date: 2025.04.01

        ' Updates:
            ' 2026.01.29 - Initiated the update log.
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    ...
    ' ==============================================================
        ...
        ' --------------------------------------------------------------
            ...
            Dim AZGives_Headers_Part1 As Variant
            Dim AZGives_Headers_Part1b As Variant
            Dim AZGives_Headers_Part2 As Variant
            Dim AZGives_Headers_Option1 As Variant
            Dim AZGives_Headers_Option2 As Variant
        ...
        ' --------------------------------------------------------------
            ...
            ' As of the most recent update, this is how the DonationSite name appears in Salesforce and Intacct.
                DonationSite = "AZ Gives Day"
                
            ' ...............................
            ' Possible Headers
            ' ...............................
                ' The original report was Part1 and Part2 combined for the column headers. As of 2025.12, "Entered By" became standard to split Part1 from Part2.
                ' These are intentionally broken out to show the variations between reports; however the final report will delete the '_Part1b' portion _
                    to standardize the process and retain all necessary headers
                
                    ' Columns A through F (Consistent in all reports)
                        AZGives_Headers_Part1 = Array("Tracking #", "Type", "Date", "Time (Arizona)", "Donor First Name", "Donor Last Name")
                    
                    ' As of 2025.12 it became a new standard, to insert "Entered By" between what was originally AZGives_Headers_{Part1} and ...{Part2} at Column G
                    ' Currently: Column G
                        AZGives_Headers_Part1b = Array("Entered By")
                    
                    ' Originally: Columns G through AY
                    ' Currently: Columns H through AZ
                        AZGives_Headers_Part2 = Array("Address", "City", "State", "Zip", "Country", "Email", "Designation", "Dedication Type", "Dedication Name", "Send Dedication Email", _
                                ...
                                "External Transaction Tracking ID", "Entered At (Arizona)", "Processing Entity Name", "Foster Care Tax Credit", "Public School Tax Credit", _
                                "Campus Affiliation (ID11580)", "Recognition (ID11683)", "Interested in Volunteering", "Volunteer Hours Pledged", "Notes")
                        
                        
                        
                    ' Define all supported AZ Gives header layouts (known report variants), accounting for the presence or absence of the 'Entered By' section.
                    ' Header values are intentionally duplicated here to preserve explicit column order and to make each supported report layout fully self-contained and easy to validate.
                        ' Option 1: No Entered By (Part1, Part2)
                            AZGives_Headers_Option1 = Array("Tracking #", "Type", "Date", "Time (Arizona)", "Donor First Name", "Donor Last Name", _
                        ...
                        ' Option 2: Entered By present (Part1, Part1b, Part2)
                            AZGives_Headers_Option2 = Array("Tracking #", "Type", "Date", "Time (Arizona)", "Donor First Name", "Donor Last Name", _
                                "Entered By", _
                                "Address", "City", "State", "Zip", "Country", "Email", "Designation", "Dedication Type", "Dedication Name", "Send Dedication Email", _
                                "Dedication Email Address", "Dedication Email Message", "Source", "Page Creator", "Donation Site", "Team Name", "Giving Event Name", "Repeats", _
                                "Payment Method", "Origin", "Amount", "Transaction Fee Rate", "Transaction Fee Cost", "Fees Covered", "Covered Cost", "Net Cost", "Net Amount", "Refund", _
                        ...
                        ' Defines the maximum expected column index for AZ Gives reports.
                            ' As of the most recent update, the furthest-right column is AZ (column 52).
                            ' Option 1: 51 columns
                            ' Option 2: 52 columns
                                'ExpectedMaxLastColumn = 52
...
'                RunConsolidationOnly = False
    ...
    ' ==============================================================
        ...
        ' --------------------------------------------------------------
            ...
            Dim ValidationAZGivesSchoolNameCol As Long
    ...
    ' Display a pre-run checklist outlining all required information the user must have available before starting the converter.
        UserResponse = MsgBox( _
                "Before starting, please confirm you have the following:" & vbCrLf & vbCrLf & _
                    "1. A report downloaded from the AZ Gives donation site." & vbCrLf & _
                    "2. The Disbursement ID from the individual disbursement page." & vbCrLf & _
                    "3. Whether an additional prize was awarded by AZ Gives and, if so, the prize amount and description name for it." & vbCrLf & vbCrLf & _
                "Are you ready to continue?", _
                vbYesNo + vbQuestion, _
                "AZ Gives – SF Converter Confirmation")
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    ...
    ' USER FILE SELECTION
    ' ==============================================================
        ' Prompt the user to select the AZ Gives disbursement report file for validation and processing.
        ...
        ' If a file is selected, its path is stored for use later in the conversion process.
            ...
            With fdFilePath
                .Title = "Select the 'AZ Gives' Report"
                ...
                If .Show <> -1 Then
                    ExitMessage = "No file selected. Please locate the 'AZ Gives' Report and try again."
...
' Update the Status Bar
    Application.StatusBar = "Evaluating File to Determine if it is an AZ Gives Report"
        ...
        ' Loop through each column header to see if it matches the expected AZ Gives column headers, to determine if it is the correct report.
            ...
            ' This supports report variations where the header row is no longer on row 1
                If AllowHeaderRowSearch = True Then
                    ...
                    ' If the column count does not match any supported layout, skip this row immediately.
                        If TempHeadersLastCol > UBound(AZGives_Headers_Option2) + 1 Then
                            GoTo NextRow
                        ElseIf TempHeadersLastCol = UBound(AZGives_Headers_Option2) + 1 Then
                            HeaderLayoutOption = 2
                        ElseIf TempHeadersLastCol = UBound(AZGives_Headers_Option1) + 1 Then
                    ...
                    ' Load the expected header array for the selected layout option.
                        ...
                        Case 1
                            CheckHeaderLayout = AZGives_Headers_Option1
                        Case 2
                            CheckHeaderLayout = AZGives_Headers_Option2
                        End Select
            ...
            ' When header search is disabled, validate only the configured HeaderRow (default is 1).
                ...
                ' If the column count does not match any supported layout, exit cleanly before continuing.
                    If TempHeadersLastCol > UBound(AZGives_Headers_Option2) + 1 Then
                        ...
                        ExitMessage = "The selected file does not match any supported AZ Gives report layout." & vbCrLf & vbCrLf & _
...
' Update the Status Bar
    ...
    ' DISBURSEMENT ID
    ' ==============================================================
        ' Prompt for the Disbursement ID shown on the AZ Gives disbursement details page.
        ...
        ' This detail can only be found on the donation site's Disbursement Details Page otherwise.
            Do
                DisbursementID = InputBox( _
                    "The file you selected was confirmed to be an 'AZ Gives' File." & vbCrLf & vbCrLf & _
                    "Please enter the Disbursement ID from the 'AZ Gives' Disbursement Page:", _
    ...
    ' ==============================================================
        ...
        ' These details can only be found on the donation site's Disbursement Details Page otherwise.
            PrizeAwarded = MsgBox("Were any prizes awarded by 'AZ Gives' for this disbursement?", _
                                    vbYesNo, _
                                    "Prizes Awarded?")
...
' Update the Status Bar
        ...
        ' --------------------------------------------------------------
            ...
            ' ADD THE ORIGINAL DONATION SITE REPORT (UNALTERED)
                If AllowReportsPullIn = True Then
                    ...
                    wsDonationSiteReportRaw.Name = "AZ Gives (" & DisbursementID & ")"
    ...
    ' ==============================================================
        ...
        ' --------------------------------------------------------------
            ' Delete the 'AZGives_Headers_Part1b' column -- "Entered By"
            ' Option 2: Column 7 (Column G)
                If HeaderLayout = "Option 2" Then
        ...
        ' --------------------------------------------------------------
            ...
            ' ...............................
                ' If AZ Gives awarded a prize for this disbursement:
                    ...
                    ' Used by Accounting to correctly reflect total funds received.
                        ...
                        ' Add in all the details for the new line item
                            ' "Tracking #"
                                wsTemp.Range("A" & TempLastRow).Value = DisbursementID & " - AZ Gives Prize"
                                    
                            ' "Date"
                                wsTemp.Range("C" & TempLastRow).Value = wsTemp.Range("AI2").Value
                                
                            ' "Source"
                                wsTemp.Range("S" & TempLastRow).Value = "BASIS Charter Schools"
                            ...
                            ' "Notes"
                                wsTemp.Range("BF" & TempLastRow).Value = "AZ Gives - Disbursement ID: " & DisbursementID & " | Adjustment: Prize | Prize Memo: " & PrizeDescription
                        End If
...
' Update the Status Bar
    ...
    ' The goal of this section is to:
      ' 1. Preserve the original AZ Gives report (do not overwrite it).
    ...
    ' Build the enhanced file name using:
      ...
      ' Total deposit-related sum for quick identification in the folder
        FileNameNew = "AZ Gives - " & Format(wsTemp.Range("AJ2").Value, "YYYY.MM.DD") & " (" & DisbursementID & ")_" & _
    ...
    ' Create a new timestamped folder
        ...
        ' Second: Create the New Folder Path variable with the appropriate naming
            FolderPathNew = FolderPathOld & "AZ Gives - for 'AR' Team - " & CurrentDateTime
...
' Update the Status Bar
    ...
    ' ==============================================================
        ...
        ' The process is to:
          ...
          ' Note: After the deletion, "SF School Name" should be in Column A, "AZ Gives School Name" in Column B, and "School Abbreviation" in Column C.
        ...
        ' --------------------------------------------------------------
            ...
            ' ...............................
                ' Search for the column header
                    Set ValidationColumnSearch = wsSchoolValidation.Rows(1).Find( _
                                                                        What:="AZ Gives School Name", _
                                                                        LookIn:=xlValues, _
                                                                        LookAt:=xlWhole, _
                                                                        MatchCase:=False)
                ' Store the column in a variable
                    ValidationAZGivesSchoolNameCol = ValidationColumnSearch.Column
                
                ' Copy the full column after all data
                    wsSchoolValidation.Columns(ValidationAZGivesSchoolNameCol).Copy wsSchoolValidation.Columns(201)
    ...
    ' ==============================================================
        ...
        ' "Donation Amount"
            wsSFImport.Range("G2").Formula = "=IF('Donation Site Report'!AE2=""Yes"",'Donation Site Report'!AB2+'Donation Site Report'!AF2,'Donation Site Report'!AB2)"
        ...
        ' "Campaign Name"
            ...
            wsSFImport.Range("N2").Formula2 = "=A2&"" ""&IF(MONTH(E2)>6,RIGHT(E2,4)&""-""&(RIGHT(E2,2)+1),IF(MONTH(F2)>8,RIGHT(E2,4)&""-""&(RIGHT(E2,2)+1),MID(E2,7,2)&RIGHT(E2,2)-1&""-""&(RIGHT(E2,2))))&"" ""&IF(LEFT(A2,5)=""BASIS"",""ATF"",""ATF AZ Gives Day Donation"")"

        ' "Donation Name" (Opportunity Name)
            wsSFImport.Range("O2").Formula = "=XLOOKUP(A2,'School Validation'!A:A,'School Validation'!C:C)&"" ATF AZ Gives Day Donation"""
        ...
        ' "Notes"
            wsSFImport.Range("R2").Formula = "=IF(ISBLANK('Donation Site Report'!AZ2),"""",'Donation Site Report'!AZ2)"
...
' Update the Status Bar
    ...
    ' ==============================================================
        ExitMessage = "The converter completed successfully. Thank you for your patience!" & vbCrLf & vbCrLf & "The Salesforce Import File is now completed for the 'AZ Gives' Report"
